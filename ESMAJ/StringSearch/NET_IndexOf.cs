...
namespace ESMAJ.StringSearch
{
    public class NET_IndexOf
    {
        ...
        public static int Search(string pattern, string source, int startIndex)
        {
            ...
            stopwatch.Start();
            int result = source.IndexOf(pattern, startIndex);
            stopwatch.Stop();
        ...
        public static List<int> Search(string pattern, string source)
        {
            ...
            Stopwatch stopwatch = new Stopwatch();
            stopwatch.Start();
            int minIndex = source.IndexOf(pattern, 0);
            if (minIndex != -1)
                result.Add(minIndex);
            while (minIndex != -1)
            {
                minIndex = source.IndexOf(pattern, minIndex + pattern.Length);
                if(minIndex != -1)
                    result.Add(minIndex);
            }
            stopwatch.Stop();
